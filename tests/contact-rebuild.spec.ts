...
test.describe('Contact Page Rebuild', () => {
  ...
  test('should load the contact page with correct metadata', async ({ page }) => {
  ...
  test('should display trust signals', async ({ page }) => {
     ...
     await expect(page.locator('p').filter({ hasText: /Licensed/i }).first()).toBeVisible();
     await expect(page.locator('p').filter({ hasText: /258533/ }).first()).toBeVisible(); // CCB
  });
});
