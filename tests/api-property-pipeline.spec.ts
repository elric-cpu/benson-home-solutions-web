...
test.describe('Property Enrichment Webhook API', () => {
  ...
  test('should reject requests without a secret header', async ({
    request,
  }) => {
  ...
  test('should accept a valid Oregon address and return enriched data', async ({
    request,
  }) => {
    const response = await request.post(ENDPOINT, {
      headers: { 'X-BHS-Webhook-Secret': 'test-secret' },
    ...
    expect(['success', 'cached']).toContain(body.status);
