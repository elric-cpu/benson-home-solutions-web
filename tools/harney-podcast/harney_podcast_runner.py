...
def render(paths,ep):
    ...
    if not fq["pass"]: record(paths,False,ep); raise PipelineError("Fidelity QC: "+" | ".join(fq["errors"]))
    ...
    if not mq["pass"]: record(paths,False,ep); raise PipelineError("Master QC: "+" | ".join(mq["errors"]))
    if current(paths,ep)["stage"]=="AUDIO FIDELITY QC": advance(paths,ep,"MASTER QC","master-qc")
    vr=record(paths,True,ep); return {"preflight":pf,"fidelity":fq,"master_qc":mq,"validation":vr}
