...
export function EmergencyActionBar() {
  ...
  return (
    <div
      className="border-oxblood/10 shadow-up animate-in slide-in-from-bottom fixed right-0 bottom-0 left-0 z-[60] border-t bg-cream/95 p-4 pb-safe backdrop-blur-md duration-300 md:hidden"
      role="complementary"
      aria-label="Emergency Actions"
    >
      <div className="mb-3 text-center">
        <p className="text-oxblood/80 text-[10px] font-bold uppercase tracking-widest">
          🚨 24/7 Emergency Dispatch: Mid-Willamette Valley & Harney County
        </p>
      </div>
      <div className="flex gap-3">
        <a href={`tel:${emergencyPhone}`} className="flex-[2]" aria-label={`Call emergency number ${emergencyPhone}`}>
          <Button
            variant="emergency"
        ...
        <a href={`sms:${emergencyPhone}?body=${smsBody}`} className="flex-1" aria-label={`Text emergency number ${emergencyPhone}`}>
          <Button
            variant="outline"
