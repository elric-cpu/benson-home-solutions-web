          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8">
            ...
            <div className="space-y-6">
              <Link href="/" className="inline-block group">
                <span className="text-2xl font-black text-cream tracking-tight group-hover:text-oxblood-light transition-colors">
              ...
              <p className="text-sm leading-relaxed max-w-xs text-cream/90">
              ...
              <div className="flex items-center gap-4">
                <a href={BUSINESS.facebook} target="_blank" rel="noopener noreferrer" className="text-cream/70 hover:text-cream transition-colors">
            ...
            <div>
              ...
              <ul className="space-y-4">
                {footerLinks.services.map((link) => (
                  <li key={link.href}>
                    <Link href={link.href} className="text-sm text-cream/80 hover:text-cream transition-colors">
                      {link.name}
                    </Link>
                  </li>
                ))}
              ...
              <div>
                ...
                <div className="space-y-4">
                  <a href={`tel:${BUSINESS.phone}`} className="block group">
                    <span className="block text-xs text-cream/70 mb-1 uppercase tracking-tighter font-bold">Office</span>
                    <span className="text-cream group-hover:text-cream-dark transition-colors font-bold">{BUSINESS.phone}</span>
                  </a>
                  <a href={`tel:${BUSINESS.afterhoursPhone}`} className="block group">
                    <span className="block text-xs text-red-200 mb-1 uppercase tracking-tighter font-bold">Emergency 24/7</span>
                    <span className="text-red-100 group-hover:text-red-200 transition-colors font-bold">{BUSINESS.afterhoursPhone}</span>
                  </a>
                  <a href={`mailto:${BUSINESS.email}`} className="block group">
                    <span className="block text-xs text-cream/70 mb-1 uppercase tracking-tighter font-bold">Email</span>
                    <span className="text-cream group-hover:text-cream-dark transition-colors font-bold">{BUSINESS.email}</span>
                  </a>
                </div>
              </div>
              <div className="p-4 rounded-xl bg-white/5 border border-white/10">
                <p className="text-xs font-bold text-cream mb-1">{BUSINESS.license}</p>
                <p className="text-[10px] uppercase tracking-widest text-cream/70 font-bold">Oregon CCB Registered</p>
              </div>
            </div>
          </div>
        </Container>
        ...
        <Container>
          <div className="flex flex-col md:flex-row justify-between items-center gap-6">
            ...
            <div className="flex gap-8">
              <Link href="/privacy" className="text-xs text-cream/70 hover:text-cream transition-colors">Privacy Policy</Link>
              <Link href="/terms" className="text-xs text-cream/70 hover:text-cream transition-colors">Terms of Service</Link>
            </div>
          </div>
        </Container>
      </div>
