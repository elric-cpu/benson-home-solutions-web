...
export function calculateServicePrice(
  ...
  const ageBracket = 
    property.age <= 10 ? '0-10' : 
    property.age <= 25 ? '11-25' : 
    property.age <= 50 ? '26-50' : '50+';
  ...
  const floodMult = service.flood_zone_multiplier[property.floodZone] || service.flood_zone_multiplier['default'] || 1.0;
  const sqftFactor = 1 + service.sqft_multiplier * Math.max(0, (property.sqft - 1000) / 1000);

  let price = service.base_price * sqftFactor * ageMult * floodMult * freqMultiplier;
