...
export const masterMarketingFlow = ai.defineFlow(
  {
    ...
    inputSchema: z.object({
      ...
      asset_type: z
        .enum(['guide', 'checklist', 'how-to', 'interactive_tool'])
        .default('guide'),
      target_url: z
        .string()
        .default('https://www.bensonhomesolutions.com/new-asset'),
    }),
  ...
  async (input) => {
    try {
      ...
      console.warn(
        '[MasterFlow] Editorial approved. Proceeding to fulfillment...',
      );

      // Step 4: Fulfillment (Parallel)
      console.warn(
        '[MasterFlow] Running Developer, Multimedia, and Outreach...',
      );

      const developerCodePromise =
        input.asset_type === 'interactive_tool' ||
        contentDraft.interactive_tool_logic
          ? webDeveloperFlow({
      ...
      const [developerCode, multimediaAssets, outreachCampaign] =
        await Promise.all([
