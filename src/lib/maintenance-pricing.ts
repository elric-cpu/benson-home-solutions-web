...
export type TierKey<S extends Segment = Segment> = keyof (typeof planData.segments)[S]['tiers']
...
export function getTierEntries(segment: Segment): MaintenanceTier[] {
  return Object.entries(planData.segments[segment].tiers).map(([key, tier]) => ({
    key,
    name: tier.name,
    price: tier.price,
...
export function getTier(segment: Segment, tierKey?: string | null): MaintenanceTier {
  const tiers = getTierEntries(segment)
  const resolvedKey = tierKey && tierKey in planData.segments[segment].tiers
    ? tierKey
...
export function getResidentialIndustryContext(homeValue?: number | null, homeAge?: number | null) {
  const percentBand =
  ...
  if (!homeValue) {
    return {
      ...
      annualHigh: null,
      note:
        'Industry budgeting rules usually start at about 1% of home value per year and climb for older homes, larger homes, and harsher climates.',
    }
  }
  ...
  return {
    ...
    note:
      'This benchmark uses the common 1%-plus maintenance budgeting rule, with higher bands for older homes that typically require more correction work.',
  }
}

export function getEmergencyRiskFrame(segment: Segment, annualPlanPrice: number) {
  const multiplier = segment === 'commercial' ? 2.4 : segment === 'church' ? 2.2 : 2
