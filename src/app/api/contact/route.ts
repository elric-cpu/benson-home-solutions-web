...
export async function POST(request: NextRequest) {
  try {
    ...
    if (body.attachmentName && !body.attachmentData) {
      return NextResponse.json({ error: 'Attachment was not uploaded correctly.' }, { status: 400 });
    }
    if (body.attachmentData && !body.attachmentName) {
      return NextResponse.json({ error: 'Please provide a name for your attachment.' }, { status: 400 });
    }
    ...
    if (body.attachmentData) {
      if (!body.attachmentType) {
        return NextResponse.json({ error: 'Attachment type is required when attachment data is provided.' }, { status: 400 });
      }
      ...
      if (actualAttachmentSize > MAX_ATTACHMENT_SIZE_BYTES) {
        return NextResponse.json({ error: 'Attachments must be 3.75MB or smaller.' }, { status: 400 });
      }
      ...
      const fileExtension = body.attachmentName?.split('.').pop()?.toLowerCase();
      const validMimeTypes = fileExtension ? allowedFileTypes[fileExtension] : undefined;

      if (!fileExtension || !validMimeTypes || !validMimeTypes.includes(body.attachmentType)) {
        return NextResponse.json({ error: 'Invalid file type or extension. Only JPG, PNG, WEBP, and PDF are allowed.' }, { status: 400 });
      }
    }
    ...
    const attachmentNote = body.attachmentName
      ...
      : null;
    const messageForDb = attachmentNote ? `${trimmedMessage}\n\nAttachment: ${attachmentNote}` : trimmedMessage;
    const hasAttachment = Boolean(body.attachmentName && normalizedAttachmentData);

    if (process.env.TURNSTILE_SECRET_KEY) {
      if (!body.turnstileToken) {
        return NextResponse.json({ error: 'Please complete the spam check and try again.' }, { status: 400 });
      }
    ...
    if (!apiKey) {
      logError(new Error('INTERNAL_API_KEY is not set.'), { context: 'Enrichment Trigger Failed' });
    } else {
      fetch(`${internalBaseUrl}/api/enrich`, {
        ...
        if (!res.ok) logError(new Error(`Enrichment failed with status: ${res.status}`), { context: 'Enrichment Trigger Failed' });
