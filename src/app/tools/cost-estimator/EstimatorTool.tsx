...
export function EstimatorTool() {
  ...
  const [result, setResult] = useState<{ low: number; high: number; formula: string } | null>(null);
  ...
  return (
    <div className="space-y-8">
      ...
      {result && (
        <Card className="bg-oxblood border-none text-cream shadow-xl">
          <CardContent className="p-8 text-center">
            <Badge variant="secondary" className="bg-cream/10 border-cream/20 text-cream mb-4 uppercase">
              March 2026 Market Range
            </Badge>
            <div className="text-5xl font-black md:text-7xl tabular-nums">
              ${result.low.toLocaleString()} - ${result.high.toLocaleString()}
            </div>
            <p className="mt-4 text-xl opacity-80">
              Estimated project cost for a {sqft} sqft {type.toLowerCase()} remodel.
            </p>
            <div className="bg-cream/5 mt-8 rounded-lg p-4 text-sm italic">
              Verification Formula: {result.formula}
            </div>
            <p className="mt-6 text-xs opacity-50 max-w-lg mx-auto">
              *Estimates include 3.4% 2026 labor index adjustment. Final pricing depends on specific site conditions and IRC-2026 compliance requirements.
            </p>
          </CardContent>
        </Card>
      )}
