...
async function ingestRecursive(
  pageId: string,
  title: string,
  visited: Set<string>,
) {
...
async function run() {
  ...
  try {
    const mainPage: any = await notion.pages.retrieve({
      page_id: NOTION_DB_KNOWLEDGE!,
    });
    const title =
      mainPage.properties.title?.title?.[0]?.plain_text || 'Operations Manual';
